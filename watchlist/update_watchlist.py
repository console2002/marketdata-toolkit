...
def _read_static_extras(path: Optional[str]) -> set[str]:
    if not path:
        return set()
    try:
...
def update_watchlist(
    *,
    ...
    static_extras_json: Optional[str] = None,
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="watchlist-update",
        description="Derive tickers.json from portfolio/trade CSVs.",
    )
